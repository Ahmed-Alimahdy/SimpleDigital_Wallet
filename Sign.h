...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	auto adm_it = Admin::adminMap.find(username);
	// Check textboxes if empty
	...
	else if (username == "Admin" && password == "@dmin")
	{
		...
		Admin_UI^ admin = gcnew Admin_UI(this, adminObj);
		admin->Show();
		textBox1->Text = "";
		textBox2->Text = "";
		this->Hide();
	}
	else if (adm_it != Admin::adminMap.end()) {
		if (adm_it->second.password == password) {
			MessageBox::Show("Sign In Successful!", "Success", MessageBoxButtons::OK, MessageBoxIcon::Information);
			Admin_UI^ admin = gcnew Admin_UI(this,adm_it->second);
			admin->Show();
			textBox1->Text = "";
			textBox2->Text = "";
			this->Hide();
